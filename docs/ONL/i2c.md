...
ONLP platform implmentations often rely on i²c capability to talk to various
...
* Make sure any implementations that use i²c headers directly are using the correct
...
* We can modify the the custom header to use the expected includes in
  ...
  This way, platforms don't have to update the i²c definitions everywhere.
